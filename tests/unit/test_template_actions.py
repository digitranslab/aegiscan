...
from aegiscan.executor import service
from aegiscan.executor.service import run_action_from_input
...
from aegiscan.types.exceptions import RegistryValidationError, AegiscanValidationError
...
async def test_template_action_fetches_nested_secrets(
    ...
    monkeypatch.setattr(config, "AEGISCAN__UNSAFE_DISABLE_SM_MASKING", True)
...
def test_template_action_definition_validates_self_reference():
    ...
    """
    with pytest.raises(AegiscanValidationError) as exc_info:
        TemplateActionDefinition(
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"user_id": "john@digi-trans.org", "service_source": "custom", "limit": 99},
            ["john@digi-trans.org", "custom", 99],
            False,
        ),
        (
            {"user_id": "john@digi-trans.org"},
            ["john@digi-trans.org", "elastic", 100],
            False,
        ),
        (
            {},
