...
from aegiscan.dsl.models import ActionStatement, RunActionInput, RunContext
from aegiscan.executor.models import DispatchActionContext
from aegiscan.executor.service import (
    _dispatch_action,
    dispatch_action_on_cluster,
    run_action_from_input,
)
from aegiscan.expressions.common import ExprContext
from aegiscan.git import GitUrl
from aegiscan.identifiers.workflow import WorkflowUUID
...
def dispatch_context():
    return DispatchActionContext(
        role=Role(type="service", service_id="aegiscan-executor"),
...
async def test_dispatch_action_basic(mock_session, basic_task_input, dispatch_context):
    with patch("aegiscan.executor.service.run_action_on_ray_cluster") as mock_ray:
...
async def test_dispatch_action_with_git_url(mock_session, basic_task_input):
    with (
        patch("aegiscan.executor.service.prepare_git_url") as mock_git_url,
        patch("aegiscan.executor.service._dispatch_action") as mock_dispatch,
        patch("aegiscan.executor.service.get_ssh_command") as mock_ssh_cmd,
    ):
...
async def test_run_action_from_input_secrets_handling(mocker, test_role):
    ...
    mocker.patch(
        "aegiscan.registry.actions.service.RegistryActionsService.with_session",
    ...
    mocker.patch(
        "aegiscan.executor.service.extract_templated_secrets",
        return_value=["args_secret1", "args_secret2"],
    )

    # Mock get_runtime_env
    mocker.patch("aegiscan.executor.service.get_runtime_env", return_value="test_env")
    ...
    auth_sandbox_mock = mocker.patch("aegiscan.executor.service.AuthSandbox")
    auth_sandbox_mock.return_value = mock_sandbox

    # Mock run_single_action to avoid actually running the action
    mocker.patch("aegiscan.executor.service.run_single_action", return_value="result")

    # Mock evaluate_templated_args
    mocker.patch("aegiscan.executor.service.evaluate_templated_args", return_value={})

    # Mock env_sandbox
    mocker.patch("aegiscan.executor.service.env_sandbox")

    # Mock flatten_secrets
    mocker.patch("aegiscan.executor.service.flatten_secrets", return_value={})
