...
from aegiscan.auth.dependencies import require_auth_type_enabled, verify_auth_type
...
async def test_verify_auth_type_not_allowed(
    ...
    mocker.patch("aegiscan.config.AEGISCAN__AUTH_TYPES", allowed_types)
...
async def test_verify_auth_type_setting_disabled(mocker: MockerFixture):
    ...
    mocker.patch("aegiscan.config.AEGISCAN__AUTH_TYPES", [AuthType.BASIC])
    mocker.patch("aegiscan.auth.dependencies.get_setting", return_value=False)
...
async def test_verify_auth_type_invalid_setting(mocker: MockerFixture):
    ...
    mocker.patch("aegiscan.config.AEGISCAN__AUTH_TYPES", [AuthType.BASIC])
    mocker.patch("aegiscan.auth.dependencies.get_setting", return_value=None)
...
async def test_verify_auth_type_success(mocker: MockerFixture):
    ...
    mocker.patch("aegiscan.config.AEGISCAN__AUTH_TYPES", [AuthType.BASIC])
    mocker.patch("aegiscan.auth.dependencies.get_setting", return_value=True)
