...
from aegiscan.api.executor import create_app
...
def rate_limited_client(monkeypatch):
    monkeypatch.setattr(config, "AEGISCAN__RATE_LIMIT_ENABLED", True)
    monkeypatch.setattr(config, "AEGISCAN__RATE_LIMIT_RATE", 1.0)  # 1 token per second
    monkeypatch.setattr(config, "AEGISCAN__RATE_LIMIT_CAPACITY", 2.0)  # Max 2 tokens
    monkeypatch.setattr(config, "AEGISCAN__RATE_LIMIT_WINDOW_SIZE", 60)
    monkeypatch.setattr(config, "AEGISCAN__RATE_LIMIT_BY_IP", True)
    monkeypatch.setattr(config, "AEGISCAN__RATE_LIMIT_BY_ENDPOINT", True)
...
def disabled_rate_limit_client(monkeypatch):
    monkeypatch.setattr(config, "AEGISCAN__RATE_LIMIT_ENABLED", False)
    app = create_app()
    app.router.lifespan = lifespan  # type: ignore
