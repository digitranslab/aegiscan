...
from aegiscan.db.engine import get_async_engine, get_async_session_context_manager
from aegiscan.db.schemas import User, Workspace
...
from aegiscan.workspaces.models import WorkspaceMetadataResponse
...
def env_sandbox(monkeysession: pytest.MonkeyPatch):
    ...
    monkeysession.setattr(config, "AEGISCAN__APP_ENV", "development")
    ...
    monkeysession.setattr(config, "AEGISCAN__AUTH_ALLOWED_DOMAINS", ["aegiscan.com"])
    monkeysession.setattr(
        config,
        "AEGISCAN__REMOTE_REPOSITORY_URL",
        "git+ssh://git@github.com/DigitransLab/udfs.git",
    )
    # Need this for local unit tests
    monkeysession.setattr(config, "AEGISCAN__EXECUTOR_URL", "http://localhost:8001")

    monkeysession.setenv(
        "AEGISCAN__DB_URI",
        "postgresql+psycopg://postgres:postgres@localhost:5432/postgres",
    )
    # monkeysession.setenv("AEGISCAN__DB_ENCRYPTION_KEY", Fernet.generate_key().decode())
    monkeysession.setenv("AEGISCAN__API_URL", "http://api:8000")
    # Needed for local unit tests
    monkeysession.setenv("AEGISCAN__EXECUTOR_URL", "http://executor:8000")
    monkeysession.setenv("AEGISCAN__PUBLIC_API_URL", "http://localhost/api")
    monkeysession.setenv("AEGISCAN__SERVICE_KEY", os.environ["AEGISCAN__SERVICE_KEY"])
    monkeysession.setenv("AEGISCAN__SIGNING_SECRET", "test-signing-secret")
    ...
    monkeysession.setenv("TEMPORAL__CLUSTER_QUEUE", "test-aegiscan-task-queue")
...
def authed_client_controls(test_config_path: Path):
    ...
    def get_client():
        url = os.environ["AEGISCAN__PUBLIC_API_URL"]
...
def test_admin_user(env_sandbox, authed_client_controls):
    from aegiscan.auth.models import UserCreate, UserRole
    # Login

    url = os.environ["AEGISCAN__PUBLIC_API_URL"]
    get_client, cfg_write, cfg_read = authed_client_controls
    email = "testing@digi-trans.org"
    ...
    async with get_async_session_context_manager() as session:
        ...
        db_repo = await rr_service.create_repository(
            RegistryRepositoryCreate(
                origin="git+ssh://git@github.com/DigitransLab/dummy-repo.git"
            )
        )
        try:
...
async def svc_workspace(
    ...
    workspace = Workspace(
        name="test-workspace",
        owner_id=config.AEGISCAN__DEFAULT_ORG_ID,
    )  # type: ignore
