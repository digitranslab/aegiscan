...
if [ ! -e ".env.example" ] ; then
  echo "${RED}No .env.example file found in current directory: $(pwd). Please download .env.example from the Aegiscan GitHub repo and rerun the env.sh script."
  exit 1
fi
env_file=".env"
...
# Replace existing values of AEGISCAN__SERVICE_KEY and AEGISCAN__SIGNING_SECRET
dotenv_replace "AEGISCAN__SERVICE_KEY" "$service_key" "$env_file"
dotenv_replace "AEGISCAN__SIGNING_SECRET" "$signing_secret" "$env_file"
dotenv_replace "AEGISCAN__DB_ENCRYPTION_KEY" "$db_fernet_key" "$env_file"
...
while true; do
    ...
    echo -e "${RED}Cannot use 0.0.0.0 as address.\nSee https://docs.aegiscan.com/self-hosting/deployment-options/docker-compose#download-configuration-files ${NC}"
done
...
dotenv_replace "AEGISCAN__APP_ENV" "$env_mode" "$env_file"
...
dotenv_replace "AEGISCAN__DB_SSLMODE" "$ssl_mode" "$env_file"

# Remove duplicate entries and leading/trailing commas
new_origins=$(echo "$new_origins" | tr ',' '\n' | sort -u | tr '\n' ',' | sed 's/^,//;s/,$//')
dotenv_replace "AEGISCAN__ALLOW_ORIGINS" "$new_origins" "$env_file"
