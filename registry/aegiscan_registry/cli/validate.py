...
from aegiscan.registry.actions.service import (
    RegistryActionsService,
    validate_action_template,
)
from aegiscan.registry.constants import DEFAULT_REGISTRY_ORIGIN
...
def template(
    ...
    mode: ValidateMode = typer.Option(
        ...
        "--mode",
        help=(
            "Default: Validate actions against actions in `aegiscan_registry`. "
            "Isolated: Validate templates only. "
            "Db: Validate templates and actions against a live Aegiscan DB."
        ),
    ),
    db_uri: str = typer.Option(
        lambda: os.getenv(
            "AEGISCAN__DB_URI",
            # Points to a locally running Aegiscan DB
            "postgresql+psycopg://postgres:postgres@localhost:5432/postgres",
        ),
        help="The database URI to use for validation. Defaults to AEGISCAN__DB_URI if set, otherwise points to a locally running Aegiscan DB.",
    ),
) -> None:
    ...
    async def main():
        if mode == ValidateMode.DB:
            ...
            config.AEGISCAN__DB_URI = db_uri
    ...
    try:
        ...
        if "nodename nor servname provided, or not known" in str(e):
            console.print(
                "The database URI is invalid. Please check your AEGISCAN__DB_URI environment variable.",
                style="bold red",
            )
        else:
