...
async def reload_registry(session: AsyncSession, role: Role):
    ...
    # Setup Aegiscan base repository
    ...
    if config.AEGISCAN__LOCAL_REPOSITORY_ENABLED:
        if not config.AEGISCAN__LOCAL_REPOSITORY_PATH:
            raise ValueError("Local repository path is not set")
        logger.info(
            "Setting up local registry repository",
            path=config.AEGISCAN__LOCAL_REPOSITORY_PATH,
        )
    ...
    if maybe_remote_url := await get_setting(
        ...
        default=config.AEGISCAN__REMOTE_REPOSITORY_URL,
    ):
