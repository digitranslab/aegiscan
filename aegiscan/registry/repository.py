...
from aegiscan_registry import RegistrySecret
...
from aegiscan.expressions.expectations import create_expectation_model
from aegiscan.expressions.validation import TemplateValidator
from aegiscan.git import GitUrl, get_git_repository_sha, parse_git_url
from aegiscan.logger import logger
from aegiscan.parse import safe_url
from aegiscan.registry.actions.models import BoundRegistryAction, TemplateAction
from aegiscan.registry.constants import (
...
from aegiscan.settings.service import get_setting
from aegiscan.ssh import SshEnv, ssh_context
from aegiscan.types.auth import Role
from aegiscan.types.exceptions import RegistryError
...
class Repository:
    ...
    def _load_base_udfs(self) -> None:
        ...
        import aegiscan_registry

        self._register_udfs_from_package(aegiscan_registry)
    ...
    async def load_from_origin(self, commit_sha: str | None = None) -> str | None:
        ...
        # Handle local git repositories
            ...
            if not config.AEGISCAN__LOCAL_REPOSITORY_ENABLED:
                raise RegistryError(
                    "Local repository is not enabled on this instance. "
                    "Please set AEGISCAN__LOCAL_REPOSITORY_ENABLED=true "
                    "and ensure AEGISCAN__LOCAL_REPOSITORY_PATH points to a valid Python package."
                )
            ...
            if not repo_path.joinpath("pyproject.toml").exists():
                ...
                if host_path := config.AEGISCAN__LOCAL_REPOSITORY_PATH:
                ...
                raise RegistryError(
                    ...
                    "Please ensure AEGISCAN__LOCAL_REPOSITORY_PATH points to a valid Python package."
            ...
            extra_args = []
            if config.AEGISCAN__APP_ENV == "production":
            ...
            allowed_domains = cast(
                ...
                await get_setting(
                    ...
                    default=config.AEGISCAN__ALLOWED_GIT_DOMAINS,
                )
                or {"github.com"},
            )
            ...
            package_name = (
                await get_setting(
                    ...
                    role=self.role,
                    # TODO: Deprecate in future version
                    default=config.AEGISCAN__REMOTE_REPOSITORY_PACKAGE_NAME,
                )
                or repo_name
            )
            logger.debug(
    ...
    def _register_udf_from_function(
        ...
        *,
        ...
        key = getattr(fn, "__aegiscan_udf_key")
        kwargs = getattr(fn, "__aegiscan_udf_kwargs")
    ...
    def _register_udfs_from_module(
        ...
        *,
        ...
        for name, obj in inspect.getmembers(module):
            ...
            is_udf = hasattr(obj, "__aegiscan_udf_key")
            has_udf_kwargs = hasattr(obj, "__aegiscan_udf_kwargs")
