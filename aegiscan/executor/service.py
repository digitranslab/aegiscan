...
from aegiscan.auth.sandbox import AuthSandbox
from aegiscan.concurrency import GatheringTaskGroup
from aegiscan.contexts import ctx_logger, ctx_role, ctx_run
from aegiscan.db.engine import get_async_engine
from aegiscan.dsl.common import context_locator, create_default_execution_context
...
from aegiscan.executor.engine import EXECUTION_TIMEOUT
from aegiscan.executor.models import DispatchActionContext, ExecutorActionErrorInfo
from aegiscan.expressions.common import ExprContext, ExprOperand
from aegiscan.expressions.eval import (
    eval_templated_object,
    extract_templated_secrets,
    get_iterables_from_expression,
)
from aegiscan.git import prepare_git_url
from aegiscan.logger import logger
from aegiscan.parse import get_pyproject_toml_required_deps, traverse_leaves
from aegiscan.registry.actions.models import BoundRegistryAction
from aegiscan.registry.actions.service import RegistryActionsService
from aegiscan.secrets.common import apply_masks_object
from aegiscan.secrets.constants import DEFAULT_SECRETS_ENVIRONMENT
from aegiscan.secrets.secrets_manager import env_sandbox
from aegiscan.ssh import get_ssh_command
...
async def run_action_from_input(input: RunActionInput, role: Role) -> Any:
    ...
    if config.AEGISCAN__UNSAFE_DISABLE_SM_MASKING:
        act_logger.warning(
...
async def run_action_on_ray_cluster(
    ...
    if config.AEGISCAN__LOCAL_REPOSITORY_ENABLED:
        local_repo_path = config.AEGISCAN__LOCAL_REPOSITORY_CONTAINER_PATH
        logger.info(
...
async def dispatch_action_on_cluster(
    ...
    Raises:
        AegiscanException: If there are errors evaluating for_each expressions or during execution
