...
class AegiscanUUID[ShortID: str](UUID):
    """Base class for prefixed Aegiscan UUIDs.
    ...
    def from_short(cls, short_id: ShortID) -> Self:
        """Create a AegiscanUUID instance from a shortened ID string.

        Args:
            short_id: The shortened ID string with prefix

        Returns:
            AegiscanUUID[ShortIDType]: A new instance created from the short ID
    ...
    def new(cls, id: Any) -> Self:
        """Coerce an ID into an instance of AegiscanUUID. Handles legacy ids."""
