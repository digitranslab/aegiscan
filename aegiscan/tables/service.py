...
from aegiscan.authz.controls import require_access_level
from aegiscan.db.schemas import Table, TableColumn
from aegiscan.identifiers import TableColumnID, TableID
from aegiscan.identifiers.workflow import WorkspaceUUID
from aegiscan.logger import logger
from aegiscan.service import BaseService
from aegiscan.tables.common import (
    handle_default_value,
    is_valid_sql_type,
    to_sql_clause,
)
from aegiscan.tables.enums import SqlType
from aegiscan.tables.models import (
    TableColumnCreate,
    TableColumnUpdate,
    TableCreate,
    TableRowInsert,
...
from aegiscan.types.auth import AccessLevel
from aegiscan.types.exceptions import AegiscanAuthorizationError, AegiscanNotFoundError
...
class TablesService(BaseService):
    ...
    def _workspace_id(self) -> WorkspaceUUID:
        ...
        if workspace_id is None:
            raise AegiscanAuthorizationError("Workspace ID is required")
    ...
    async def get_table(self, table_id: TableID) -> Table:
        ...
        table = result.first()
        if table is None:
            raise AegiscanNotFoundError("Table not found")
        return table
    ...
    async def get_table_by_name(self, table_name: str) -> Table:
        ...
        Raises:
            AegiscanNotFoundError: If the table does not exist
        """
        ws_id = self._workspace_id()
        ...
        table = result.first()
        if table is None:
            raise AegiscanNotFoundError(f"Table '{table_name}' not found")
        return table
    ...
    async def create_table(self, params: TableCreate) -> Table:
        ...
        Raises:
            AegiscanAuthorizationError: If user lacks required permissions
    ...
    async def get_column(
        ...
        column = result.first()
        if column is None:
            raise AegiscanNotFoundError("Column not found")
        return column
    ...
    async def get_row(self, table: Table, row_id: UUID) -> Any:
        ...
        if row is None:
            raise AegiscanNotFoundError(f"Row {row_id} not found in table {table.name}")
        return row
    ...
    async def update_row(
        ...
        Raises:
            AegiscanNotFoundError: If the row does not exist
        """
        schema_name = self._get_schema_name()
        conn = await self.session.connection()
