...
from aegiscan.auth.models import SessionRead, UserUpdate
...
from aegiscan.db.schemas import AccessToken, User
from aegiscan.identifiers import SessionID, UserID
...
class OrgService(BaseService):
    ...
    async def delete_member(self, user_id: UserID) -> None:
        ...
        Raises:
            AegiscanAuthorizationError: If the user is a superuser and cannot be deleted.
        """
        user = await self.get_member(user_id)
        if user.is_superuser:
            raise AegiscanAuthorizationError("Cannot delete superuser")
    ...
    async def update_member(self, user_id: UserID, params: UserUpdate) -> User:
        ...
        Raises:
            AegiscanAuthorizationError: If the user is a superuser and cannot be updated.
        """
        user = await self.get_member(user_id)
        if user.is_superuser:
            raise AegiscanAuthorizationError("Cannot update superuser")
        async with self._manager() as user_manager:
