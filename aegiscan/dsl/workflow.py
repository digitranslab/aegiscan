...
with workflow.unsafe.imports_passed_through():
    ...
    import aegiscan_registry  # noqa
    from pydantic import TypeAdapter, ValidationError

    from aegiscan import identifiers
    from aegiscan.concurrency import GatheringTaskGroup
    from aegiscan.contexts import ctx_logger, ctx_role, ctx_run
    from aegiscan.dsl.action import (
        DSLActivities,
    ...
    from aegiscan.dsl.enums import CoreActions, FailStrategy, LoopStrategy
    ...
    from aegiscan.dsl.scheduler import DSLScheduler
    from aegiscan.dsl.validation import (
        ValidateTriggerInputsActivityInputs,
        validate_trigger_inputs_activity,
    )
    from aegiscan.executor.service import evaluate_templated_args, iter_for_each
    from aegiscan.expressions.common import ExprContext
    from aegiscan.expressions.eval import eval_templated_object
    from aegiscan.identifiers.workflow import WorkflowExecutionID, WorkflowID
    from aegiscan.logger import logger
    from aegiscan.types.exceptions import (
        AegiscanCredentialsError,
        AegiscanDSLError,
        AegiscanException,
        AegiscanExpressionError,
        AegiscanNotFoundError,
        AegiscanValidationError,
    )
    from aegiscan.validation.models import ValidationResult
    from aegiscan.workflow.executions.models import ErrorHandlerWorkflowInput
    from aegiscan.workflow.management.definitions import (
        get_workflow_definition_activity,
    )
    from aegiscan.workflow.management.management import WorkflowsManagementService
    from aegiscan.workflow.management.models import (
        GetErrorHandlerWorkflowIDActivityInputs,
        GetWorkflowDefinitionActivityInputs,
        ResolveWorkflowAliasActivityInputs,
    )
    from aegiscan.workflow.schedules.models import GetScheduleActivityInputs
...
non_retryable_error_types = [
    ...
    # Aegiscan
    AegiscanException.__name__,
    AegiscanExpressionError.__name__,
    AegiscanValidationError.__name__,
    AegiscanDSLError.__name__,
    AegiscanCredentialsError.__name__,
    # Temporal
...
class DSLWorkflow:
    ...
    async def _run_workflow(self, args: DSLRunArgs) -> Any:
        ...
        except AegiscanExpressionError as e:
            raise ApplicationError(
    ...
    async def _validate_action(self, task: ActionStatement) -> None:
        ...
        if not result.ok:
            raise ApplicationError(
                ...
                result.detail,
                non_retryable=True,
                type=AegiscanValidationError.__name__,
            )
    ...
    async def _get_error_handler_workflow_id(
        ...
        This is done by checking if the error is a AegiscanValidationError or
        AegiscanExpressionError.
        """
        return await workflow.execute_activity(
