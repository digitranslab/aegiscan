...
from aegiscan.dsl.enums import EdgeType, FailStrategy, LoopStrategy
...
from aegiscan.dsl.view import RFEdge, RFGraph, RFNode, TriggerNode, UDFNode, UDFNodeData
from aegiscan.expressions import patterns
...
from aegiscan.identifiers.workflow import AnyWorkflowID, WorkflowUUID
from aegiscan.logger import logger
from aegiscan.parse import traverse_leaves
from aegiscan.types.auth import Role
from aegiscan.types.exceptions import AegiscanDSLError
...
class DSLInput(BaseModel):
    ...
    def validate_structure(self) -> Self:
        if not self.actions:
            raise AegiscanDSLError("At least one action must be defined.")
        ...
        if n_entrypoints == 0:
            raise AegiscanDSLError("No entrypoints found")
    ...
    def from_yaml(path: str | Path | SpooledTemporaryFile) -> DSLInput:
        ...
        if isinstance(path, str | Path):
            ...
            raise AegiscanDSLError(f"Invalid file/path type {type(path)}")
