...
from aegiscan.auth.models import UserRole
from aegiscan.auth.users import is_unprivileged, optional_current_active_user
from aegiscan.authz.service import AuthorizationService
...
from aegiscan.identifiers import InternalServiceID
from aegiscan.logger import logger
from aegiscan.types.auth import AccessLevel, Role

oauth2_scheme = OAuth2PasswordBearer(tokenUrl="token", auto_error=False)
api_key_header_scheme = APIKeyHeader(name="x-aegiscan-service-key", auto_error=False)
...
def get_role_from_user(
    ...
    service_id: InternalServiceID = "aegiscan-api",
) -> Role:
    return Role(
        type="user",
...
async def _authenticate_service(
    ...
    service_role_id = request.headers.get("x-aegiscan-role-service-id")
    if service_role_id is None:
        msg = "Missing x-aegiscan-role-service-id header"
        logger.error(msg)
        raise HTTP_EXC(msg)
    if service_role_id not in config.AEGISCAN__SERVICE_ROLES_WHITELIST:
        msg = f"x-aegiscan-role-service-id {service_role_id!r} invalid or not allowed"
        logger.error(msg)
        raise HTTP_EXC(msg)
    if api_key != os.environ["AEGISCAN__SERVICE_KEY"]:
        logger.error("Could not validate service key")
        raise CREDENTIALS_EXCEPTION
    role_params = {
        "type": "service",
        "service_id": service_role_id,
        "access_level": AccessLevel[request.headers["x-aegiscan-role-access-level"]],
    }
    if (user_id := request.headers.get("x-aegiscan-role-user-id")) is not None:
        role_params["user_id"] = user_id
    if (ws_id := request.headers.get("x-aegiscan-role-workspace-id")) is not None:
