...
from aegiscan.api.common import bootstrap_role
from aegiscan.auth.models import UserCreate, UserRole, UserUpdate
from aegiscan.db.adapter import (
    SQLModelAccessTokenDatabaseAsync,
    SQLModelUserDatabaseAsync,
)
from aegiscan.db.engine import get_async_session, get_async_session_context_manager
from aegiscan.db.schemas import AccessToken, OAuthAccount, User
...
class UserManager(UUIDIDMixin, BaseUserManager[User, uuid.UUID]):
    ...
    async def validate_password(self, password: str, user: User) -> None:
        if len(password) < config.AEGISCAN__AUTH_MIN_PASSWORD_LENGTH:
            raise InvalidPasswordException(
                f"Password must be at least {config.AEGISCAN__AUTH_MIN_PASSWORD_LENGTH} characters long"
            )
    ...
    async def validate_email(self, email: str) -> None:
        allowed_domains = cast(
            ...
            await get_setting(
                ...
                default=list(config.AEGISCAN__AUTH_ALLOWED_DOMAINS),
            ),
        )
...
cookie_transport = CookieTransport(
    ...
    cookie_secure=config.AEGISCAN__API_URL.startswith("https"),
)
...
class FastAPIUserWithLogoutRouter(FastAPIUsers[models.UP, models.ID]):
    def get_logout_router(
        ...
        requires_verification: bool = config.AEGISCAN__AUTH_REQUIRE_EMAIL_VERIFICATION,
    ) -> APIRouter:
        """
