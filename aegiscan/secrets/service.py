...
from aegiscan.db.schemas import BaseSecret, OrganizationSecret, Secret
from aegiscan.identifiers import SecretID
from aegiscan.logger import logger
from aegiscan.registry.constants import GIT_SSH_KEY_SECRET_NAME
...
from aegiscan.secrets.enums import SecretType
from aegiscan.secrets.models import (
    SecretCreate,
    SecretKeyValue,
    SecretSearch,
    SecretUpdate,
...
class SecretsService(BaseService):
    ...
    def __init__(self, session: AsyncSession, role: Role | None = None):
        ...
        try:
            self._encryption_key = os.environ["AEGISCAN__DB_ENCRYPTION_KEY"]
        except KeyError as e:
            raise KeyError("AEGISCAN__DB_ENCRYPTION_KEY is not set") from e
    ...
    async def get_secret_by_name(
        ...
        Raises:
            AegiscanNotFoundError: If no secret is found with the given name/environment or if multiple secrets are found
        """
    ...
    async def list_org_secrets(
        ...
        stmt = select(OrganizationSecret).where(
            OrganizationSecret.owner_id == config.AEGISCAN__DEFAULT_ORG_ID
        )
        if types:
