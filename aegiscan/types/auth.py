...
from aegiscan.identifiers import InternalServiceID, UserID, WorkspaceID
...
class Role(BaseModel):
    ...
    def to_headers(self) -> dict[str, str]:
        headers = {
            "x-aegiscan-role-type": self.type,
            "x-aegiscan-role-service-id": self.service_id,
            "x-aegiscan-role-access-level": self.access_level.name,
        }
        if self.user_id is not None:
            headers["x-aegiscan-role-user-id"] = str(self.user_id)
        if self.workspace_id is not None:
            headers["x-aegiscan-role-workspace-id"] = str(self.workspace_id)
        return headers
