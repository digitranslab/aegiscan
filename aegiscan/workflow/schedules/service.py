...
from aegiscan.auth.credentials import TemporaryRole
from aegiscan.db.schemas import Schedule
from aegiscan.identifiers import ScheduleID, WorkflowID
...
from aegiscan.types.exceptions import (
    ...
    AegiscanServiceError,
)
from aegiscan.workflow.schedules import bridge
...
class WorkflowSchedulesService(BaseService):
    ...
    async def create_schedule(self, params: ScheduleCreate) -> Schedule:
        ...
        # Set the role for the schedule as the aegiscan-runner
        with TemporaryRole(
            type="service",
            user_id=str(owner_id),
            service_id="aegiscan-schedule-runner",
        ) as sch_role:
            try:
                ...
                raise AegiscanServiceError(msg) from e
            logger.info(
                "Created schedule",
                handle_id=handle.id,
                workflow_id=params.workflow_id,
    ...
    async def get_schedule(self, schedule_id: ScheduleID) -> Schedule:
        ...
        try:
            ...
            raise AegiscanNotFoundError(f"Schedule {schedule_id} not found") from e
    ...
    async def update_schedule(
        ...
        try:
            ...
            raise AegiscanNotFoundError(msg) from e
