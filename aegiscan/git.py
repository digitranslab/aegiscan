...
from aegiscan.ssh import SshEnv
from aegiscan.types.auth import Role
from aegiscan.types.exceptions import AegiscanSettingsError
...
async def prepare_git_url(role: Role | None = None) -> GitUrl | None:
    ...
    allowed_domains_setting = await get_setting_cached(
        ...
        # TODO: Deprecate in future version
        # Must be hashable
        default=frozenset(config.AEGISCAN__ALLOWED_GIT_DOMAINS),
    )
    ...
    try:
        ...
        raise AegiscanSettingsError(str(e)) from e
    git_url.ref = sha
    return git_url
