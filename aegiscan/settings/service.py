...
from aegiscan.db.schemas import OrganizationSetting
from aegiscan.logger import logger
from aegiscan.secrets.encryption import decrypt_value, encrypt_value
from aegiscan.service import BaseService
from aegiscan.settings.constants import PUBLIC_SETTINGS_KEYS, SENSITIVE_SETTINGS_KEYS
from aegiscan.settings.models import (
    AppSettingsUpdate,
    AuthSettingsUpdate,
    BaseSettingsGroup,
    GitSettingsUpdate,
...
class SettingsService(BaseService):
    ...
    def __init__(self, session: AsyncSession, role: Role | None = None):
        ...
        try:
            self._encryption_key = SecretStr(os.environ["AEGISCAN__DB_ENCRYPTION_KEY"])
...
def get_setting_override(key: str) -> Any | None:
    ...
    return os.environ.get(f"AEGISCAN__SETTING_OVERRIDE_{key.upper()}")
