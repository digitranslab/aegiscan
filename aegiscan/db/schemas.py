...
from aegiscan.identifiers import OwnerID, action, id_factory
...
class Webhook(Resource, table=True):
    ...
    @property
    def secret(self) -> str:
        secret = os.getenv("AEGISCAN__SIGNING_SECRET")
        if not secret:
            raise ValueError("AEGISCAN__SIGNING_SECRET is not set")
    ...
    def url(self) -> str:
        ...
        return f"{config.AEGISCAN__PUBLIC_API_URL}/webhooks/{short_wf_id}/{self.secret}"
...
class RegistryAction(Resource, table=True):
    ...
    Template actions loaded from aegiscan base can be cloned but not edited.
