...
def _get_db_uri(driver: Literal["psycopg", "asyncpg"] = "psycopg") -> str:
    ...
    if config.AEGISCAN__DB_USER and config.AEGISCAN__DB_PASS__ARN:
        logger.info("Retrieving database password from AWS Secrets Manager...")
        try:
            session = boto3.session.Session()
            client = session.client(service_name="secretsmanager")
            response = client.get_secret_value(SecretId=config.AEGISCAN__DB_PASS__ARN)
        ...
        uri = get_connection_string(
            username=config.AEGISCAN__DB_USER,
            password=password,
            host=config.AEGISCAN__DB_ENDPOINT,  # type: ignore
            port=config.AEGISCAN__DB_PORT,  # type: ignore
            database=config.AEGISCAN__DB_NAME,  # type: ignore
            driver=driver,
        )
    ...
    elif config.AEGISCAN__DB_USER and config.AEGISCAN__DB_PASS:
        uri = get_connection_string(
            username=config.AEGISCAN__DB_USER,
            password=config.AEGISCAN__DB_PASS,
            host=config.AEGISCAN__DB_ENDPOINT,  # type: ignore
            port=config.AEGISCAN__DB_PORT,  # type: ignore
            database=config.AEGISCAN__DB_NAME,  # type: ignore
            driver=driver,
...
def _create_db_engine() -> Engine:
    if config.AEGISCAN__APP_ENV == "production":
        # Postgres
        sslmode = os.getenv("AEGISCAN__DB_SSLMODE", "require")
        engine_kwargs = {
            "pool_timeout": 30,
            "pool_recycle": 3600,
