...
from aegiscan.authz.models import OwnerType
from aegiscan.db.schemas import Membership, Ownership, User, Workspace
from aegiscan.identifiers import OwnerID, UserID, WorkspaceID
from aegiscan.service import BaseService
from aegiscan.types.auth import AccessLevel
from aegiscan.types.exceptions import AegiscanException, AegiscanManagementError
from aegiscan.workspaces.models import SearchWorkspacesParams, UpdateWorkspaceParams
...
class WorkspaceService(BaseService):
    ...
    async def admin_list_workspaces(self, limit: int | None = None) -> list[Workspace]:
        ...
        if limit is not None:
            if limit <= 0:
                raise AegiscanException("List workspace limit must be greater than 0")
    ...
    async def create_workspace(
        ...
        name: str,
        *,
        owner_id: OwnerID = config.AEGISCAN__DEFAULT_ORG_ID,
