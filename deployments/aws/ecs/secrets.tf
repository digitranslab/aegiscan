...
# 1. AEGISCAN__DB_ENCRYPTION_KEY
# 2. AEGISCAN__SERVICE_KEY
# 3. AEGISCAN__SIGNING_SECRET
#
# Optional secrets:
# 1. OAUTH_CLIENT_ID
...
### Required secrets
data "aws_secretsmanager_secret" "aegiscan_db_encryption_key" {
  arn = var.aegiscan_db_encryption_key_arn
}

data "aws_secretsmanager_secret" "aegiscan_service_key" {
  arn = var.aegiscan_service_key_arn
}

data "aws_secretsmanager_secret" "aegiscan_signing_secret" {
  arn = var.aegiscan_signing_secret_arn
}

### Optional secrets

# Aegiscan authentication
...
data "aws_secretsmanager_secret_version" "aegiscan_db_encryption_key" {
  secret_id = data.aws_secretsmanager_secret.aegiscan_db_encryption_key.id
}

data "aws_secretsmanager_secret_version" "aegiscan_service_key" {
  secret_id = data.aws_secretsmanager_secret.aegiscan_service_key.id
}

data "aws_secretsmanager_secret_version" "aegiscan_signing_secret" {
  secret_id = data.aws_secretsmanager_secret.aegiscan_signing_secret.id
}
...
data "aws_secretsmanager_secret" "aegiscan_db_password" {
...
data "aws_secretsmanager_secret_version" "aegiscan_db_password" {
  secret_id = data.aws_secretsmanager_secret.aegiscan_db_password.id
}
...
locals {
  aegiscan_base_secrets = [
    {
      name      = "AEGISCAN__SERVICE_KEY"
      valueFrom = data.aws_secretsmanager_secret_version.aegiscan_service_key.arn
    },
    {
      name      = "AEGISCAN__SIGNING_SECRET"
      valueFrom = data.aws_secretsmanager_secret_version.aegiscan_signing_secret.arn
    },
    {
      name      = "AEGISCAN__DB_ENCRYPTION_KEY"
      valueFrom = data.aws_secretsmanager_secret_version.aegiscan_db_encryption_key.arn
    },
  ]
  ...
  aegiscan_api_secrets = concat(
    local.aegiscan_base_secrets,
