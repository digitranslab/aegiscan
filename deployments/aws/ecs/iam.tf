...
resource "aws_iam_policy" "secrets_access" {
  name        = "AegiscanSecretsAccessPolicy"
  description = "Policy for accessing Aegiscan secrets"
    ...
    Statement = [
      {
        ...
        Resource = compact([
          var.aegiscan_db_encryption_key_arn,
          var.aegiscan_service_key_arn,
          var.aegiscan_signing_secret_arn,
...
resource "aws_iam_policy" "task_secrets_access" {
  ...
  name        = "AegiscanTaskSecretsAccessPolicy"
  description = "Policy for accessing Aegiscan secrets at runtime"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
...
resource "aws_iam_policy" "temporal_secrets_access" {
  ...
  name        = "AegiscanTemporalSecretsAccessPolicy"
...
resource "aws_iam_role" "worker_execution" {
  name               = "AegiscanWorkerExecutionRole"
  assume_role_policy = data.aws_iam_policy_document.assume_role.json
}
...
resource "aws_iam_role" "api_worker_task" {
  name               = "AegiscanAPIWorkerTaskRole"
  assume_role_policy = data.aws_iam_policy_document.assume_role.json
}

resource "aws_iam_role_policy" "api_worker_task_db_access" {
  name = "AegiscanAPIWorkerDBAccessPolicy"
...
resource "aws_iam_role" "temporal_execution" {
  count              = var.disable_temporal_autosetup ? 0 : 1
  name               = "AegiscanTemporalExecutionRole"
  assume_role_policy = data.aws_iam_policy_document.assume_role.json
}
...
resource "aws_iam_role" "temporal_task" {
  count              = var.disable_temporal_autosetup ? 0 : 1
  name               = "AegiscanTemporalTaskRole"
  assume_role_policy = data.aws_iam_policy_document.assume_role.json
}

resource "aws_iam_role_policy" "temporal_task_db_access" {
  count = var.disable_temporal_autosetup ? 0 : 1
  name  = "AegiscanTemporalDBAccessPolicy"
...
resource "aws_iam_role" "caddy_execution" {
  name               = "AegiscanCaddyExecutionRole"
  assume_role_policy = data.aws_iam_policy_document.assume_role.json
}
...
resource "aws_iam_policy" "cloudwatch_logs" {
  name        = "AegiscanCloudWatchLogsPolicy"
    ...
    Statement = [
      {
        ...
        Resource = "${aws_cloudwatch_log_group.aegiscan_log_group.arn}:*"
      }
    ]
  })
}
...
resource "aws_iam_policy" "temporal_ui_secrets_access" {
  ...
  name        = "AegiscanTemporalUISecretsAccessPolicy"
...
resource "aws_iam_role" "temporal_ui_execution" {
  count              = var.disable_temporal_ui ? 0 : 1
  name               = "AegiscanTemporalUIExecutionRole"
  assume_role_policy = data.aws_iam_policy_document.assume_role.json
}
...
resource "aws_iam_role" "temporal_ui_task" {
  count              = var.disable_temporal_ui ? 0 : 1
  name               = "AegiscanTemporalUITaskRole"
  assume_role_policy = data.aws_iam_policy_document.assume_role.json
}
