...
clean-images:
	docker images "aegiscan*" -q | xargs -r docker rmi
dev:
...
_check-cli:
	#!/usr/bin/env sh
	set -e
	command -v aegiscan >/dev/null 2>&1 || { echo "Error: Aegiscan CLI is not installed" >&2; exit 1; }

gen-api: _check-cli
	LOG_LEVEL=ERROR aegiscan dev generate-spec --update-docs

gen-integrations: _check-cli
	aegiscan dev gen-integrations

gen-functions: _check-cli
	aegiscan dev gen-functions
