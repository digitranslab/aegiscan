...
<Warning>
  ...
  assigned to the environment variable: `AEGISCAN__DB_ENCRYPTION_KEY`.
</Warning>
...
<Steps>
  <Step title="Download latest environment variables migration script and template">
    ...
    and template `.env.example` to ensure compatibility with the new Aegiscan
    version.

    ```
    curl -o env-migration.sh https://raw.githubusercontent.com/DigitransLab/aegiscan/0.27.3/env-migration.sh
...
Integrations are not automatically updated when you update the Aegiscan version.
You will need to manually sync the Aegiscan registry repository from the **Action Registry** page:

![Sync registry](/img/self-hosting/sync-registry.png)

## Terraform Deployments

All of Aegiscan's Cloud deployments are configured using Terraform.
To update Aegiscan, you can download the latest Terraform stack from Aegiscan's GitHub repo and run `terraform apply`.
...
Your data is still intact. Make sure you backup the core Aegiscan database:

- For Docker Compose and AWS EC2 deployments, this would be the `core-db` volume in Aegiscan's Docker stack.
...
If you need live migration support, contact us on [Discord](https://discord.gg/n3GF4qxFU8) or email [founders@digi-trans.org](mailto:founders@digi-trans.org)!
