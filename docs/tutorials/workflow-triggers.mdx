...
<Note>
  This tutorial does not cover how to expose your Aegiscan instance to the public internet.

  - If you're running Aegiscan locally, you'll need use a tunneling service like [ngrok](https://ngrok.com/) to receive webhook requests from the public internet.
  - If you're running Aegiscan on AWS Fargate, Aegiscan is automatically exposed to the public internet via an Application Load Balancer.
</Note>
...
Every Aegiscan workflow comes with a single `Trigger` action.
...
```bash
https://<aegiscan-public-url>/api/webhooks/<workflow-id>/<webhook-secret>
```
