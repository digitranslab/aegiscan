---
title: Quickstart
description: Everything you need to know to get started with Aegiscan.
icon: rocket
---
...
- Have Aegiscan [installed](/quickstart/install)
...
<AccordionGroup>
  ...
  <Accordion title="Call an external weather API" icon="cloud-sun">
    ...
    Aegiscan uses YAML to define action inputs.
...
Actions are the building blocks of Aegiscan workflows.
Aegiscan has two main types of actions:
...
Find out more in Aegiscan's [core actions](/integrations/core) and [tools](/integrations/tools) docs.
...
<Note>
  Conditional expressions are one of the most powerful features in Aegiscan.
...
- Learn about Aegiscan's [core actions](/integrations/core).
