---
title: Core Actions
description: Building blocks of Aegiscan workflows and action templates.
icon: cube
---
...
<Tip>
  All core actions are open source and available in [Aegiscan's GitHub repository](https://github.com/DigitransLab/aegiscan/tree/main/registry/aegiscan_registry/core).
</Tip>
...
Core actions are the building blocks of Aegiscan workflows and action templates.
...
`core` actions are the most commonly used actions in Aegiscan.
They include:
...
Aegiscan makes it easy to poll APIs with long-running operations using the `core.http_poll` action.
...
<Accordion title="Example: URLScan" icon="globe">
  ...
  <Steps>
    <Step title="Create URLScan secret">
      Add URLScan API key to Aegiscan's built-in secrets manager.
...
<Note>
  Aegiscan supports self-hosted LLMs such as `llama`, `mistral`, and `deepseek`.
