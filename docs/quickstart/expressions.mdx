...
Aegiscan supports the following expression contexts:
...
<Accordion title="Webhook example" icon="globe">
  ...
  ```bash
  export WEBHOOK_URL=https://aegiscan.com/api/webhooks/<webhook_id>/<secret>
...
Aegiscan comes with a built-in secrets manager.
...
Aegiscan will automatically replace the expression with the secret value at runtime.
...
Aegiscan supports inline functions in the `FN` context.
