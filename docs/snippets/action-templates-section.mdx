...
**Action Templates** are custom integrations built on Aegiscan's YAML-based DSL (domain specific language).
...
<Note>
  Only data specified in the `returns` field of the template is logged in Aegiscan workflows.
...
Templates support all actions defined in the **Aegiscan Registry**.
...
This means that templates have full support for Aegiscan's powerful [inline functions](/quickstart/expressions).
For example:
