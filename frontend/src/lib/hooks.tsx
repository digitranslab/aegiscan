...
import { retryHandler, AegiscanApiError } from "@/lib/errors"
  ...
  const {
    ...
    queryFn: async () => {
      ...
      try {
        ...
        throw new Error(
          "Unable to fetch authentication settings. This could be a network issue with the Aegiscan API."
        )
      }
    },
  })
...
export function useWorkflowManager(filter?: WorkflowFilter) {
  ...
  const { mutateAsync: createWorkflow } = useMutation({
    ...
    onError: (error: AegiscanApiError) => {
      switch (error.status) {
        case 400:
          toast({
...
export function useWorkspaceManager() {
  ...
  const getLastWorkspaceId = () =>
    Cookies.get("__aegiscan:workspaces:last-viewed")
  const setLastWorkspaceId = (id?: string) =>
    Cookies.set("__aegiscan:workspaces:last-viewed", id ?? "")
  const clearLastWorkspaceId = () =>
    Cookies.remove("__aegiscan:workspaces:last-viewed")
...
export function useRegistryActions(versions?: string[]) {
  ...
  const {
    ...
    onError: (error: AegiscanApiError) => {
      ...
      const apiError = error as AegiscanApiError
...
export function useListRows({ tableId, workspaceId }: TablesListRowsData) {
  const {
    ...
    error: rowsError,
  } = useQuery<TableRowRead[], AegiscanApiError>({
