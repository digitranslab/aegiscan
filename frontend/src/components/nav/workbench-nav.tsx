...
import { AegiscanApiError } from "@/lib/errors"
  ...
  const handleSubmit = async (values: TWorkflowControlsForm) => {
    ...
    try {
      ...
      if (error instanceof ApiError) {
        const aegiscanError = error as AegiscanApiError
        console.error("Error details", aegiscanError.body.detail)
        switch (aegiscanError.status) {
          case 400:
            toast({
            ...
            setManualTriggerErrors(
              aegiscanError.body.detail as Record<string, string>
            )
            break
          default:
